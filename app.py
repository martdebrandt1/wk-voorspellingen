...
# Officiële FIFA bracket structuur voor R32
# Elke match definieert: positie1 (winner/runner-up van groep) en positie2 (welke groepen mogelijk zijn voor 3rd place)
# Gebaseerd op de officiële WK 2026 bracket
R32_STRUCTURE = [
    # Match 1-8: Winners vs 3rd places (met toegestane groepen)
    ...
    # Match 9-12: Runners-up vs Runners-up
    ...
    # Match 13-16: Winners vs Runners-up
...
(function() {
   ...
   var R32_STRUCTURE = %R32_STRUCTURE_JSON%;
    var thirdPlaceAssignments = {}; // matchIndex -> group letter
    ...
    function buildKnockout() {
        ...
        // First: build the 3rd-place assignment UI
    ...
    function buildThirdPlaceAssignment(container) {
        ...
        html += '<p class="drag-hint">Voor elke wedstrijd hieronder, kies welke van de doorgegane #3 teams die wedstrijd speelt. Elk team kan maar 1x gekozen worden.</p>';

        // Find matches that need a 3rd place
        for (var i = 0; i < R32_STRUCTURE.length; i++) {
            ...
            html += '<select class="third-assign-select" data-match="' + i + '" style="width:100%;padding:10px;border-radius:8px;background:rgba(255,255,255,0.1);color:#fff;border:2px solid rgba(255,255,255,0.2);font-size:1em;">';
            ...
            for (var j = 0; j < selectedThirds.length; j++) {
                ...
                if (match.slot2.allowed.indexOf(grp) >= 0) {
                    var team = groupPredictions[grp][2];
                    html += '<option value="' + grp + '">' + team + ' (Groep ' + grp + ')</option>';
                }
            }
            html += '</select>';
            html += '</div>';
        ...
        html += '<div class="btn-group"><button class="btn btn-primary" id="btn-confirm-thirds" type="button">Bevestig & Ga naar Wedstrijden &#8594;</button></div>';
        div.innerHTML = html;
        container.appendChild(div);
        ...
        for (var s = 0; s < selects.length; s++) {
            selects[s].addEventListener('change', updateThirdAssignmentOptions);
        }
        document.getElementById('btn-confirm-thirds').addEventListener('click', confirmThirdAssignments);
    }

    function updateThirdAssignmentOptions() {
        // Get all currently selected groups
        var selects = document.querySelectorAll('.third-assign-select');
        var usedGroups = [];
        for (var i = 0; i < selects.length; i++) {
            if (selects[i].value) usedGroups.push(selects[i].value);
        }
        // Disable already-used groups in other selects
        for (var i = 0; i < selects.length; i++) {
            var currentVal = selects[i].value;
            var options = selects[i].querySelectorAll('option');
            for (var j = 0; j < options.length; j++) {
                if (options[j].value === '') continue;
                if (options[j].value === currentVal) {
                    options[j].disabled = false;
                } else if (usedGroups.indexOf(options[j].value) >= 0) {
    ...
    function confirmThirdAssignments() {
        ...
        for (var i = 0; i < selects.length; i++) {
            if (!selects[i].value) {
                alert('Wijs alle 8 wedstrijden een #3 team toe!');
                return;
            }
            var matchIdx = parseInt(selects[i].getAttribute('data-match'));
            thirdPlaceAssignments[matchIdx] = selects[i].value;
        }
        // Verify all 8 thirds are used
        var usedGroups = [];
        for (var k in thirdPlaceAssignments) usedGroups.push(thirdPlaceAssignments[k]);
        if (usedGroups.length !== selectedThirds.length) {
            alert('Niet alle #3 teams zijn toegewezen!');
            return;
        }

        // Remove assignment UI and build R32 matches
    ...
    function resolveTeam(slot) {
        ...
        if (slot.type === 'third') {
            // Resolved via thirdPlaceAssignments at match build time
            return 'TBD';
        }
        return 'TBD';
    }
    ...
    function buildR32Matches() {
        ...
        for (var i = 0; i < R32_STRUCTURE.length; i++) {
            ...
            var team1 = resolveTeam(m.slot1);
            var team2;
            if (m.slot2.type === 'third') {
                var assignedGroup = thirdPlaceAssignments[i];
                team2 = assignedGroup ? groupPredictions[assignedGroup][2] : 'TBD';
            } else {
                team2 = resolveTeam(m.slot2);
            }
            matches.push({ team1: team1, team2: team2 });
        }
        renderRound(container, '1/16 Finales (Ronde van 32)', matches, 'r32');
    }
